...
def model(image, params, weights, biases):
    expvals = pnp.array(circuit(image, params))
    expvals_mapped = (expvals + 1) / 2                      # test
    logits = pnp.dot(weights, expvals_mapped) + biases      # test
    #logits = pnp.dot(weights, expvals) + biases
    return logits
